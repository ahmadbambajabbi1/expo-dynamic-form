...
const FileUploadController = ({ controller, field, form }: PropsType) => {
  ...
  // Function to get MIME type from base64 or file extension
  const getMimeTypeFromFile = (fileName: string, base64?: string): string => {
    // Try to detect from base64 header first
    if (base64) {
  ...
  const handleDocumentPick = async () => {
    try {
      ...
      let newFiles: FileInfo[] = await Promise.all(
        result.assets.map(async (asset) => {
          let base64String = "";
          let finalMimeType = asset.mimeType || getFileTypeFromUrl(asset.uri);

          try {
            // Convert to base64 for images and small files
            if (isImage(finalMimeType) || asset.size! < 5 * 1024 * 1024) {
              // 5MB limit for base64
              base64String = await convertToBase64(asset.uri);
              // Update MIME type based on base64 if needed
              finalMimeType =
                getMimeTypeFromFile(asset.name || "", base64String) ||
                finalMimeType;
            }
          } catch (error) {
            console.warn("Failed to convert file to base64:", error);
            // Continue without base64 - for large files or if conversion fails
          }
    ...
    try {
      ...
      let newFiles: FileInfo[] = await Promise.all(
        result.assets.map(async (asset) => {
          ...
          // If base64 is not available from ImagePicker, convert manually
          if (!base64String) {
            try {
              base64String = await convertToBase64(asset.uri);
            } catch (error) {
          ...
          const fileName =
            asset.fileName ||
            `image-${Date.now()}.${asset.uri.split(".").pop()}`;
          const mimeType = getMimeTypeFromFile(fileName, base64String);
    ...
    return (
      <View key={`${file.uri}-${index}`} style={styles.filePreview}>
        ...
        <View style={styles.fileInfo}>
          ...
          <Text style={styles.fileSize}>{formatFileSize(file.size)}</Text>
          {file.base64 && (
            <Text style={styles.base64Status}>✓ Ready for upload</Text>
          )}
        </View>
