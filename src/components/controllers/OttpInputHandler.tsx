...
const OttpInputHandler = ({ verifyingDataProps, apiOptions }: PropsType) => {
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    // Combined OTP for form submission
  ...
  const handleResend = async () => {
    if (minutes > 0) return; // Prevent resend if timer not expired

    setResend(true);
    setMinutes(50); // Reset timer

    // Implement your resend logic here
    // This could be a call to your API to resend the verification code
  };
